...
//! piece of information in some Eris source code, a token. Tokens are sort
//! of like the "word"s in Eris. They're meaningful in the sense that things
            ...
            return match c {
                ...
                '!' => {
                    if self.consume('=') {
                        ...
                        self.create_token(TokenKind::Error("Expected to find `=`".to_owned()), 1)
                    }
                }
                '>' => {
                    if self.consume('=') {
...
mod tests {
    ...
    fn lex_string(string: &str) -> Vec<TokenKind> {
        ...
        while !lexer.at_end() {
            tokens.push(lexer.next_token().0);
        }
    ...
    fn test_keywords() {
        let tokens = lex_string(
            "and case else false for fun if in import let mut not or pub return true while",
        );
        assert_eq!(
            tokens,
            &[
    ...
    fn test_errors() {
        ...
        assert_eq!(
            invalid_bang.first().unwrap(),
            &TokenKind::Error("Expected to find `=`".to_owned())
        );
        assert_eq!(
